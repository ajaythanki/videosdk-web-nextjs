...
const CommandContainer = () => {
  ...
  const [commandRecords, setCommandRecords] = useState([]);
  const [currentUserId, setCurrentUserId] = useState(0);
  const [commandReceivers, setCommandReceivers] = useState([]);
  const [command, setCommandUser] = useState(null);
          ...
          if (length > 0) {
            ...
            if (
              ...
              if (Array.isArray(lastRecord.message)) {
                lastRecord.message.push(payload.text);
              } else {
                lastRecord.message = [lastRecord.message, payload.text];
              }
            } else {
      ...
      <div className="chat-wrap">
        ...
        <div className="chat-message-box">
          <Form.Control
            ...
            onKeyPress={(e) => e.key === 'Enter' && onCommandInput()}
