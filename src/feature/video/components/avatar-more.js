...
const isUseVideoPlayer = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('useVideoPlayer') === '1' : false;
    ...
    ({ key }) => {
      if (key === 'volume') {
        ...
        mediaStream?.screenshotVideo(userId).then((blob) => {
          if (blob) {
            downloadFile(blob, `screenshot-${userId}-${new Date().getTime()}.png`);
          }
        });
      }
